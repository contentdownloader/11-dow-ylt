...
export interface DownloadRequest {
  url: string;
  platform: 'instagram' | 'facebook' | 'test'; // 'test' included for internal consistency, though removed from UI
...
// Simplified and more focused downloadFile function (triggers browser download)
...
export async function checkServerHealth(): Promise<boolean> {
  // Using AbortController for a proper timeout for production readiness
  const controller = new AbortController();
  const id = setTimeout(() => controller.abort(), 5000); // 5 second timeout

  try {
    const response = await fetch(`${API_BASE_URL}/health`, { signal: controller.signal });
    clearTimeout(id); // Clear the timeout if fetch resolves/rejects before timeout
    ...
    return isOnline;
  } catch (error: any) { // Type 'any' because AbortError specific property might not be on 'Error' type
    clearTimeout(id);
    if (error.name === 'AbortError') {
      logger.error('Server health check timed out:', error);
    } else {
      logger.error('Server health check failed (network or unexpected):', error);
    }
    return false;
  }
}
...
export async function getDownloadedFiles(): Promise<DownloadedFile[]> {
  ...
  try {
    ...
    // Returning empty array is often user-friendlier for lists in case of fetch failure
    return [];
  }
}
